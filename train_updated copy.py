...
try:
    ...
    # Defer hard failure until nibabel is actually needed (allows --help to work without nibabel)
    nib = None
class Config:
    ...
    def __init__(self):
        ...
        # GPU info (populated even if CUDA unavailable)
        try:
            if self.device.startswith("cuda") and torch.cuda.is_available():
                props = torch.cuda.get_device_properties(0)
                self.vram_gb = float(props.total_memory / (1024 ** 3))
        ...
        self.num_workers = 0 if os.name == "nt" else min(8, cpu_count * 2)
        ...
        self.unet_num_res_units = 2
        self.unet_dropout = 0.1
        self.unet_norm = "batch"
        self.unet_norm_overridden = False
        ...
        if self.device.startswith("cuda"):
            ...
            self.num_workers = min(8, os.cpu_count() or 8)
            self.cache_rate_train = 0.25
            self.cache_rate_val = 1.0
            self.sw_batch_size = 2
            # VRAM-aware profiles
            if self.vram_gb >= 46:
                self.roi_size = (112, 224, 224)
                self.batch_size = 2
                self.accumulation_steps = 1
                self.num_samples = 4
                self.unet_channels = (64, 128, 256, 512, 1024)
                self.unet_num_res_units = 2
                self.unet_dropout = 0.1
                self.unet_norm = "batch"
                ...
                self.accumulation_steps = 1
                self.num_samples = 4
                self.unet_channels = (48, 96, 192, 384, 768)
                self.unet_norm = "batch" if self.batch_size >= 2 else "instance"
                self.sw_batch_size = 2
        if self.device == "mps":
            # Default to lighter settings on Mac GPUs to avoid OOM
            self.batch_size = 1
            self.accumulation_steps = 1
            self.use_attention = False
            self.compile_model = False

            # Reduce memory pressure from patch sampling / caching / loaders
            self.roi_size = (64, 128, 128)
            self.num_samples = 2
            self.cache_rate_train = min(self.cache_rate_train, 0.05)
            self.cache_rate_val = min(self.cache_rate_val, 0.10)

            # DataLoader multiprocessing on macOS can inflate memory; keep modest
            self.num_workers = min(self.num_workers, 2)
            self.persistent_workers = False
            self.prefetch_factor = 2

            # Smaller model width + safer normalization for batch_size=1
            self.unet_channels = (16, 32, 64, 128, 256)
    ...
    def apply_mps_safety(self) -> None:
        """Clamp settings for Apple MPS to avoid out-of-memory and BN instability."""
        if self.device != "mps":
            return

        # Always run batch=1 on MPS for large 3D volumes
        self.batch_size = 1
        self.accumulation_steps = max(1, int(getattr(self, "accumulation_steps", 1)))

        # Prefer stability over speed on MPS
        self.use_attention = False
        self.compile_model = False

        # Patch sampling + ROI
        self.roi_size = tuple(getattr(self, "roi_size", (64, 128, 128)))
        if len(self.roi_size) == 3:
            z, y, x = self.roi_size
            if z > 80 or y > 160 or x > 160:
                self.roi_size = (64, 128, 128)

        self.num_samples = int(getattr(self, "num_samples", 2))
        if self.num_samples > 2:
            self.num_samples = 2

        # Cache rates
        self.cache_rate_train = min(float(getattr(self, "cache_rate_train", 0.05)), 0.10)
        self.cache_rate_val = min(float(getattr(self, "cache_rate_val", 0.10)), 0.25)

        # DataLoader
        self.num_workers = min(int(getattr(self, "num_workers", 0)), 2)
        self.persistent_workers = False
        self.prefetch_factor = int(getattr(self, "prefetch_factor", 2))

        # Model width and normalization
        if tuple(getattr(self, "unet_channels", ())) == (32, 64, 128, 256, 512):
            self.unet_channels = (16, 32, 64, 128, 256)

        if str(getattr(self, "unet_norm", "batch")).lower() == "batch":
            self.unet_norm = "instance"
    ...
    def from_args(cls, args: argparse.Namespace):
        ...
        if cfg.dataset_preset == "imagecas":
            ...
            # Force conservative model defaults for ImageCAS to avoid unexpected OOMs
            cfg.unet_channels = (32, 64, 128, 256, 512)
            cfg.unet_strides = (2, 2, 2, 2)
            cfg.unet_num_res_units = 2
        ...
        if getattr(args, "unet_norm", None):
            cfg.unet_norm = args.unet_norm
            cfg.unet_norm_overridden = True
        ...
        if getattr(args, "imagecas_root", None):
            ...
            cfg.data_root = cfg.imagecas_root
        ...
        if getattr(args, "use_attention", None) is not None:
            cfg.use_attention = bool(args.use_attention)
        if getattr(args, "amp", None) is not None:
            cfg.amp = bool(args.amp)
        if getattr(args, "compile_model", None) is not None:
        ...
        if getattr(args, "sw_batch_size", None) is not None:
            cfg.sw_batch_size = int(args.sw_batch_size)
        cfg.apply_mps_safety()
        if not cfg.unet_norm_overridden:
            cfg.unet_norm = "batch" if cfg.batch_size >= 2 else "instance"
        return cfg
...
def log_device_info(logger: logging.Logger, device_str: str) -> None:
    try:
        import torch
        ...
        logger.info(f"Device selected: {device_str}")
        logger.info(f"CUDA available: {cuda_ok}")
        logger.info(f"MPS built: {mps_built}, MPS available: {mps_ok}")
        if device_str == "mps":
            logger.info("✅ Using Apple GPU via MPS.")
...
def build_transforms(config: Config, mode: str = "train"):
    ...
    if mode == "train":
        ...
        # Deterministic validation: center-crop around label after padding to roi_size.
        augmentations = [
            SpatialPadd(keys=["image", "label"], spatial_size=config.roi_size),
            CropByCenterd(keys=["image", "label"], label_key="label", roi_size=config.roi_size),
            ResizeWithPadOrCropd(keys=["image", "label"], spatial_size=config.roi_size),
        ]
...
def build_model(config: Config, logger: logging.Logger):
    ...
    if config.use_attention:
        if config.device == "mps":
            logger.warning("AttentionUnet disabled on MPS to reduce memory usage; using UNet.")
            ...
            try:
                ...
                logger.info("✅ Using MONAI AttentionUnet")
            except Exception as e:
                logger.warning(f"AttentionUnet unavailable ({e}); falling back to UNet.")
                model = UNet(
                    spatial_dims=3,
                    in_channels=1,
                    out_channels=1,
    ...
    if config.compile_model and hasattr(torch, "compile"):
        if config.device == "mps":
            logger.warning("MPS compilation unstable; skipping")
        else:
            try:
...
def compute_class_weights(train_files: List[Dict], logger: logging.Logger, device: str, pos_weight_cap: float = 30.0) -> torch.Tensor:
    "Compute pos_weight for handling class imbalance, with cap to reduce false positives"
    ...
    uncapped_ratio = float(neg_vox / (pos_vox + 1e-9))
    pos_weight_val = min(uncapped_ratio, pos_weight_cap)

    logger.info(f"  Positive voxels: {int(pos_vox):,}")
    logger.info(f"  Negative voxels: {int(neg_vox):,}")
    logger.info(f"  Uncapped ratio (neg/pos): {uncapped_ratio:.2f}")
    logger.info(f"  Capped pos_weight (cap={pos_weight_cap:.1f}): {pos_weight_val:.2f}")

    return torch.tensor([pos_weight_val], device=device)


def build_loss_function(pos_weight, logger: logging.Logger, use_focal: bool = False, focal_gamma: float = 2.0, focal_alpha: float = 0.25, tversky_alpha: float = 0.7, tversky_beta: float = 0.3):
    """Stable hybrid loss for sparse vessels: Dice + (BCE or Focal) + Tversky.
    Tversky alpha/beta favor FP/ FN weighting respectively (defaults set to penalize false positives).
    """
    ...
    # Choose BCE or Focal loss based on flag
    if use_focal:
        try:
            ...
            loss_fn_name = f"FocalLoss(gamma={focal_gamma}, alpha={focal_alpha})"
            bce = focal
        except (ImportError, TypeError):
            logger.warning(f"MONAI FocalLoss not available or incompatible; falling back to BCEWithLogits.")
            bce = nn.BCEWithLogitsLoss(pos_weight=pos_weight)
            loss_fn_name = "BCEWithLogits(pos_weight)"
    else:
        bce = nn.BCEWithLogitsLoss(pos_weight=pos_weight)
        loss_fn_name = "BCEWithLogits(pos_weight)"
    ...
    def _combo(logit, target_tensor):
        return (
            0.4 * dice(logit, target_tensor)
            + 0.4 * bce(logit, target_tensor)
            + 0.2 * tversky(logit, target_tensor)
        )

    def combined_loss(logits, target):
        # Multi-scale if deep supervision is enabled
        if isinstance(logits, (list, tuple)):
            total_loss = None
            weights = [0.6, 0.3, 0.1][: len(logits)]
            for logit, weight in zip(logits, weights):
                scaled_target = nn.functional.interpolate(
                    target, size=logit.shape[2:], mode="nearest"
                )
                weighted = float(weight) * _combo(logit, scaled_target)
                total_loss = weighted if total_loss is None else total_loss + weighted
            return total_loss if total_loss is not None else torch.tensor(0.0, device=target.device)

        # Single scale
        return _combo(logits, target)

    logger.info(f"Loss: 40% Dice + 40% {loss_fn_name} + 20% Tversky")
...
class Trainer:
    ...
    def __init__(self, config: Config, logger: logging.Logger):
        ...
        self.use_amp = self.use_cuda and bool(getattr(self.config, "amp", self.use_cuda))
        if self.use_cuda:
            try:
            ...
            self.scaler = None
        self.non_blocking = self.use_cuda
        self.ckpt_best = None
        self.ckpt_last = None
    ...
    def train_epoch(self, model, loader, optimizer, loss_fn, scheduler, epoch):
        ...
        for step, batch in enumerate(pbar, start=1):
            ...
            if accumulation_counter % accumulation_steps == 0:
                ...
                optimizer.zero_grad(set_to_none=True)
                did_update = True
            if did_update and scheduler is not None:
                scheduler.step()
                effective_updates += 1
        ...
        def prefetch_batches():
            for batch in loader:
                with torch.cuda.stream(stream):
                    batch["image"] = batch["image"].to(self.config.device, non_blocking=True)
                    batch["label"] = batch["label"].to(self.config.device, non_blocking=True)
                yield batch
    ...
    def validate_epoch(self, model, loader, loss_fn, metrics, epoch):
        ...
        model.eval()
        sw_batch_size = int(getattr(self.config, "sw_batch_size", 1))
        running_loss = 0.0
        n = 0
        metric_values = {}
            ...
            for batch in tqdm(loader, desc=f"Val Epoch {epoch}", leave=False):
                ...
                try:
                    ...
                    logits = sliding_window_inference(
                        ...
                        overlap=0.25 if self.config.device == "mps" else 0.5,
                        mode="constant" if self.config.device == "mps" else "gaussian"
                    )
                except Exception as e:
    ...
    def train(self, model, train_loader, val_loader, optimizer, scheduler, loss_fn, metrics, start_epoch=0, writer=None):
        ...
        for epoch in range(start + 1, end_epoch + 1):
            train_loss = self.train_epoch(model, train_loader, optimizer, loss_fn, scheduler, epoch)
...
def main(args):
    ...
    log_device_info(logger, config.device)
    ...
    if config.device.startswith("cuda"):
        deterministic_flag = getattr(torch.backends.cudnn, "deterministic", False)
        torch.backends.cudnn.benchmark = not deterministic_flag
        try:
    ...
    if getattr(args, "detect_anomaly", False):
        ...
        logger.info(
            ...
            bool(getattr(config, "amp", False)),
        )

    logger.info(f"Model compilation (torch.compile): {config.compile_model}")

    # Log system and GPU RAM
    try:
        import psutil
        vm = psutil.virtual_memory()
        logger.info(f"System RAM: {vm.total / (1024**3):.1f} GB total, {vm.available / (1024**3):.1f} GB available")
    except ImportError:
        try:
            result = subprocess.run(["free", "-h"], capture_output=True, text=True)
            if result.returncode == 0:
                logger.info(f"System RAM (free -h):\n{result.stdout}")
        except Exception:
            pass

    if config.device.startswith("cuda") and torch.cuda.is_available():
        try:
            total_memory, reserved_memory = torch.cuda.mem_get_info()
            logger.info(f"GPU RAM: {(reserved_memory - total_memory) / (1024**3):.1f} GB reserved, {total_memory / (1024**3):.1f} GB available on device 0")
        except Exception as e:
    ...
    if config.dataset_preset == "imagecas" and getattr(args, "imagecas_root", None):
        data_root = Path(args.imagecas_root)
    elif args.data_root:
    ...
    elif data_root.is_dir() and config.dataset_preset == "imagecas" and any(data_root.glob("*.zip")):
        data_root = data_manager.extract_dataset(data_root)
    ...
    # Use args values for caching and DataLoader (which may override config defaults)
    cache_train_val = args.cache_train if hasattr(args, 'cache_train') and args.cache_train is not None else config.cache_rate_train
    cache_val_val = args.cache_val if hasattr(args, 'cache_val') and args.cache_val is not None else config.cache_rate_val
    num_workers_val = args.num_workers if hasattr(args, 'num_workers') and args.num_workers is not None else config.num_workers
    prefetch_val = args.prefetch_factor if hasattr(args, 'prefetch_factor') and args.prefetch_factor is not None else getattr(config, 'prefetch_factor', 2)
    persistent_val = args.persistent_workers if hasattr(args, 'persistent_workers') else getattr(config, 'persistent_workers', False)
    pos_weight_cap_val = args.pos_weight_cap if hasattr(args, 'pos_weight_cap') else 30.0

    logger.info(
        "Effective config: device=%s gpu=%s vram_gb=%.1f roi_size=%s batch_size=%d accumulation_steps=%d num_samples=%d num_workers=%d prefetch=%d persistent=%s cache_train=%.2f cache_val=%.2f pos_weight_cap=%.1f channels=%s sw_batch_size=%d norm=%s attention=%s compile=%s amp=%s",
        config.device,
        getattr(config, "gpu_name", ""),
        float(getattr(config, "vram_gb", 0.0)),
        config.roi_size,
        int(config.batch_size),
        int(config.accumulation_steps),
        int(getattr(config, "num_samples", 4)),
        int(num_workers_val),
        int(prefetch_val),
        bool(persistent_val),
        float(cache_train_val),
        float(cache_val_val),
        float(pos_weight_cap_val),
        str(config.unet_channels),
        int(getattr(config, "sw_batch_size", 1)),
        str(config.unet_norm),
        bool(getattr(config, "use_attention", False)),
        bool(getattr(config, "compile_model", False)),
        bool(getattr(config, "amp", False)),
    )

    # Use args values for caching (which may override config defaults)
    cache_train_rate = args.cache_train if hasattr(args, 'cache_train') and args.cache_train is not None else config.cache_rate_train
    cache_val_rate = args.cache_val if hasattr(args, 'cache_val') and args.cache_val is not None else config.cache_rate_val
    num_workers_val = args.num_workers if hasattr(args, 'num_workers') and args.num_workers is not None else config.num_workers

    logger.info("Creating training dataset (cache_rate=%.2f)...", cache_train_rate)
    t0 = time.perf_counter()
    train_ds = CacheDataset(
        data=train_files,
        transform=train_transforms,
        cache_rate=cache_train_rate,
        num_workers=num_workers_val
    )
    logger.info("Training dataset ready in %.1fs", time.perf_counter() - t0)

    logger.info("Creating validation dataset (cache_rate=%.2f)...", cache_val_rate)
    t0 = time.perf_counter()
    val_ds = CacheDataset(
        data=val_files,
        transform=val_transforms,
        cache_rate=cache_val_rate,
        num_workers=num_workers_val
    )
    logger.info("Validation dataset ready in %.1fs", time.perf_counter() - t0)

    # Use args values for DataLoader settings (may override config defaults)
    num_workers_train = args.num_workers if hasattr(args, 'num_workers') and args.num_workers is not None else config.num_workers
    prefetch_factor_val = args.prefetch_factor if hasattr(args, 'prefetch_factor') and args.prefetch_factor is not None else getattr(config, 'prefetch_factor', 2)
    persistent_workers_val = args.persistent_workers if hasattr(args, 'persistent_workers') else getattr(config, 'persistent_workers', False)
    ...
    if num_workers_train > 0:
        loader_kwargs_train["persistent_workers"] = bool(persistent_workers_val)
        loader_kwargs_train["prefetch_factor"] = int(prefetch_factor_val)
        loader_kwargs_train["worker_init_fn"] = make_worker_init_fn(args.seed)
    train_loader = DataLoader(train_ds, **loader_kwargs_train)

    val_num_workers = max(1, num_workers_train // 2) if num_workers_train > 0 else 0
    loader_kwargs_val = dict(
        batch_size=1,
        shuffle=False,
    ...
    if val_num_workers > 0:
        loader_kwargs_val["persistent_workers"] = bool(persistent_workers_val)
        loader_kwargs_val["prefetch_factor"] = int(prefetch_factor_val)
        loader_kwargs_val["worker_init_fn"] = make_worker_init_fn(args.seed + 17)
    val_loader = DataLoader(val_ds, **loader_kwargs_val)
    ...
    pos_weight_cap_val = args.pos_weight_cap if hasattr(args, 'pos_weight_cap') else 30.0
    pos_weight = compute_class_weights(train_files, logger, config.device, pos_weight_cap=pos_weight_cap_val)
    focal_gamma_val = args.focal_gamma if hasattr(args, 'focal_gamma') else 2.0
    focal_alpha_val = args.focal_alpha if hasattr(args, 'focal_alpha') else 0.25
    tversky_alpha_val = args.tversky_alpha if hasattr(args, 'tversky_alpha') else 0.7
    tversky_beta_val = args.tversky_beta if hasattr(args, 'tversky_beta') else 0.3
    loss_fn = build_loss_function(
        ...
        tversky_alpha=tversky_alpha_val,
        tversky_beta=tversky_beta_val,
    )
    ...
    scheduler = torch.optim.lr_scheduler.SequentialLR(
        ...
        schedulers=[
            ...
            torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(
                optimizer,
                T_0=config.scheduler_t0 * steps_per_epoch,
    ...
    logger.info(f"Scheduler: Warmup ({warmup_steps} steps) + CosineAnnealing")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to ImageCAS directory containing zip parts (e.g., /workspace/datasets/imagecas_v3).",
    )
    parser.add_argument(
        "--dataset",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Number of patches sampled per volume for RandCropByPosNegLabeld.",
    )
    parser.add_argument(
        "--sw_batch_size",
        type=int,
        default=None,
        help="Sliding-window inference batch size during validation.",
    )
    parser.add_argument(
        "--unet_channels",
        type=str,
    ...
    # DataLoader worker / prefetch arguments are defined later with RAM-safe defaults.
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=None,
        help="Enable MONAI AttentionUnet.",
    )
    parser.add_argument(
        "--no-attention",
        dest="use_attention",
        action="store_false",
        default=None,
        help="Disable AttentionUnet (use vanilla UNet).",
    )
    parser.add_argument(
        "--amp",
        dest="amp",
        action="store_true",
        default=None,
        help="Enable CUDA mixed precision (AMP).",
    )
    parser.add_argument(
        "--no-amp",
        dest="amp",
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="Fraction of the training set to cache in memory (0.0-1.0). Default 0.0 for RAM safety."
    )
    parser.add_argument(
        "--cache_val",
        type=float,
        default=0.0,
        help="Fraction of the validation set to cache in memory (0.0-1.0). Default 0.0 for RAM safety."
    )
    parser.add_argument(
        "--cache_rate_train",
        type=float,
        default=None,
        help="(Deprecated: use --cache_train) Fraction of the training set to cache in memory (0.0-1.0)."
    )
    parser.add_argument(
        "--cache_rate_val",
        type=float,
        default=None,
        help="(Deprecated: use --cache_val) Fraction of the validation set to cache in memory (0.0-1.0)."
    )
    parser.add_argument(
        "--num_workers",
        type=int,
        default=2,
        help="DataLoader worker count. Default 2 for RAM safety."
    )
    parser.add_argument(
        "--prefetch_factor",
        type=int,
        default=2,
        help="DataLoader prefetch factor (only used if num_workers > 0). Default 2."
    )
    parser.add_argument(
        "--persistent_workers",
        action="store_true",
        default=False,
        help="Enable persistent DataLoader worker processes (default False for RAM safety)."
    )
    parser.add_argument(
        "--pos_weight_cap",
        type=float,
        default=30.0,
        help="Cap for pos_weight to prevent extreme class imbalance artifacts. Default 30.0."
    )
    parser.add_argument(
        "--use_focal_loss",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=float,
        default=2.0,
        help="Focal loss gamma parameter for modulating loss."
    )
    parser.add_argument(
        "--focal_alpha",
        type=float,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.7,
        help="Tversky alpha (penalize false positives). Default 0.7"
    )
    parser.add_argument(
        "--tversky_beta",
        type=float,
        default=0.3,
        help="Tversky beta (penalize false negatives). Default 0.3"
    )
    parser.add_argument(
        "--resume",
        nargs="?",
